...
int brisbane_hexagon_launch(int dim, size_t off, size_t ndr) {
  int ret = -1;
  switch (brisbane_hexagon_kernel_idx) {
    case 0: ret = (*saxpy)(saxpy_args.Z, saxpy_args.Zlen, saxpy_args.A, saxpy_args.X, saxpy_args.Xlen, saxpy_args.Y, saxpy_args.Ylen, off, ndr); break;
  }
  printf("[%s:%d] idx[%d] ret[%d]\n", __FILE__, __LINE__, brisbane_hexagon_kernel_idx, ret);
